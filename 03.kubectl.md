...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem kubernetes/client/bin/kubectl root@${node_ip}:/opt/k8s/bin/
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "mkdir -p ~/.kube"
    scp -i ~/.ssh/long-20200508001.pem kubectl.kubeconfig root@${node_ip}:~/.kube/config
  done
```