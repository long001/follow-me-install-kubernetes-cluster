...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "mkdir -p /etc/kubernetes/cert"
    scp -i ~/.ssh/long-20200508001.pem kubernetes*.pem root@${node_ip}:/etc/kubernetes/cert/
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem encryption-config.yaml root@${node_ip}:/etc/kubernetes/
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem audit-policy.yaml root@${node_ip}:/etc/kubernetes/audit-policy.yaml
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem proxy-client*.pem root@${node_ip}:/etc/kubernetes/cert/
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem kube-apiserver-${node_ip}.service root@${node_ip}:/etc/systemd/system/kube-apiserver.service
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "mkdir -p ${K8S_DIR}/kube-apiserver"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "systemctl daemon-reload && systemctl enable kube-apiserver && systemctl restart kube-apiserver"
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "systemctl status kube-apiserver |grep 'Active:'"
  done
```
...
Kubernetes master is running at https://172.19.70.103:6443
...
tcp        0      0 172.19.70.103:6443     0.0.0.0:*               LISTEN      101442/kube-apiserv
```
