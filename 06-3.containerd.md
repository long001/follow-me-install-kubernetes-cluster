...
注意：
1. 如果没有特殊指明，本文档的所有操作均在 long-k8s01 节点上执行。
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem containerd/bin/*  crictl  cni-plugins/*  runc  root@${node_ip}:/opt/k8s/bin
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "chmod a+x /opt/k8s/bin/* && mkdir -p /etc/cni/net.d"
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "mkdir -p /etc/containerd/ ${CONTAINERD_DIR}/{root,state}"
    scp -i ~/.ssh/long-20200508001.pem containerd-config.toml root@${node_ip}:/etc/containerd/config.toml
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem containerd.service root@${node_ip}:/etc/systemd/system
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "systemctl enable containerd && systemctl restart containerd"
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem crictl.yaml root@${node_ip}:/etc/crictl.yaml
  done
```