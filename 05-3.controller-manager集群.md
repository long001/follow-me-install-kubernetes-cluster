...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem kube-controller-manager*.pem root@${node_ip}:/etc/kubernetes/cert/
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    ...
    scp -i ~/.ssh/long-20200508001.pem kube-controller-manager-${node_ip}.kubeconfig root@${node_ip}:/etc/kubernetes/kube-controller-manager.kubeconfig
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem kube-controller-manager-${node_ip}.service root@${node_ip}:/etc/systemd/system/kube-controller-manager.service
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "mkdir -p ${K8S_DIR}/kube-controller-manager"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "systemctl daemon-reload && systemctl enable kube-controller-manager && systemctl restart kube-controller-manager"
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "systemctl status kube-controller-manager|grep Active"
  done
```
...
tcp        0      0 172.19.70.103:10252    0.0.0.0:*               LISTEN      108977/kube-control
```
...
``` bash
$ curl -s --cacert /opt/k8s/work/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem https://172.19.70.103:10252/metrics |head
...
metadata:
  annotations:
    control-plane.alpha.kubernetes.io/leader: '{"holderIdentity":"long-k8s-01_3854f4ab-2434-441c-85d4-b978810bc56c","leaseDurationSeconds":15,"acquireTime":"2020-06-22T11:23:14Z","renewTime":"2020-06-22T11:29:21Z","leaderTransitions":0}'
  creationTimestamp: "2020-06-22T11:23:14Z"
  name: kube-controller-manager
  namespace: kube-system
  resourceVersion: "764"
  selfLink: /api/v1/namespaces/kube-system/endpoints/kube-controller-manager
  uid: 077e3669-3f83-4aa6-b760-50685ecb7a76
```
