...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem etcd-v3.4.3-linux-amd64/etcd* root@${node_ip}:/opt/k8s/bin
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "mkdir -p /etc/etcd/cert"
    scp -i ~/.ssh/long-20200508001.pem etcd*.pem root@${node_ip}:/etc/etcd/cert/
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i ~/.ssh/long-20200508001.pem etcd-${node_ip}.service root@${node_ip}:/etc/systemd/system/etcd.service
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "mkdir -p ${ETCD_DATA_DIR} ${ETCD_WAL_DIR}"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "systemctl daemon-reload && systemctl enable etcd && systemctl restart etcd " &
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i ~/.ssh/long-20200508001.pem root@${node_ip} "systemctl status etcd|grep Active"
  done
```
...
``` bash
>>> 172.19.70.103
https://172.19.70.103:2379 is healthy: successfully committed proposal: took = 2.756451ms
>>> 172.19.70.104
https://172.19.70.104:2379 is healthy: successfully committed proposal: took = 2.025018ms
>>> 172.19.70.105
https://172.19.70.105:2379 is healthy: successfully committed proposal: took = 2.335097ms
```
...
+----------------------------+------------------+---------+---------+-----------+------------+-----------+------------+--------------------+--------+
| https://172.19.70.103:2379 | 267e5d46e65464bf |   3.4.3 |   20 kB |     false |      false |         2 |          8 |                  8 |        |
| https://172.19.70.104:2379 | 84b6c12517978e80 |   3.4.3 |   20 kB |      true |      false |         2 |          8 |                  8 |        |
| https://172.19.70.105:2379 | 57891458f626bc6c |   3.4.3 |   20 kB |     false |      false |         2 |          8 |                  8 |        |
+----------------------------+------------------+---------+---------+-----------+------------+-----------+------------+--------------------+--------+
```
+ 可见，当前的 leader 为 172.19.70.104。